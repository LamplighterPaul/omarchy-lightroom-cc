#!/usr/bin/env python3
"""Build the experimental startup patch from pinned Wine source, without installing it."""
import hashlib
import json
import os
...
BUILD = DATA / 'build/d2d1'
COMMIT = '2cac6ccf33c0807f374dc96f5a20e35a2da86157'
PATCH = REPO / 'patches/d2d1-colormanagement-experimental.patch'
env = os.environ.copy()
...
with (BUILD / 'build.log').open('w') as log:
    ...
    for target in ('__tooldeps__', 'dlls/d2d1/x86_64-windows/d2d1.dll'):
        run(['make', '-j8', target], cwd=BUILD, stdout=log, stderr=log)
output = DATA / 'patches/d2d1.dll'
output.parent.mkdir(parents=True, exist_ok=True)
data = bytearray((BUILD / 'dlls/d2d1/x86_64-windows/d2d1.dll').read_bytes())
...
metadata = {'wine_commit': COMMIT, 'patch_sha256': hashlib.sha256(PATCH.read_bytes()).hexdigest(),
            'dll_sha256': hashlib.sha256(data).hexdigest(), 'experimental_passthrough': True}
